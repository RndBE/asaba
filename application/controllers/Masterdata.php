...
class MasterData extends CI_Controller
{
	...
	function livedata()
	{
		if ($this->session->userdata('logged_in')) {
			...
			$data['status_siaga'] = $this->db->where('log_master',$id_logger)->get('set_ews')->row_array();
			$data['konten'] = 'konten/back/analisa_liveawlr';
	...
	public function pilihposawlr()
	{
		...
		$q_pos = $this->db->query("SELECT * FROM t_logger INNER JOIN t_lokasi ON t_logger.lokasi_logger = t_lokasi.idlokasi where kategori_log='3' or kategori_log='2'");
	...
	function analisa()
	{
		...
		if ($this->session->userdata('logged_in')) {
			...
			$data['konten'] = 'konten/back/masterdata';
			$this->load->view('template_admin/site', $data);
		} else {
			redirect('login');
		}
	...
	function editsiaga($logger)
	{
		$data = array(
			'se1' => $this->input->post('se1'),
			'se2' => $this->input->post('se2'),
			'se3' => $this->input->post('se3'),
		);
		$this->db->where('log_master',$logger)->update('set_ews', $data);
		redirect('masterdata/livedata');
	}
}
