...
<html lang="id">
	<head>
		...
		<style>
			...
			.fullscreen-target:fullscreen #plot{height:100vh;min-height:100vh}
		</style>
	</head>
	<body>
						...
						<div class="card-body">
							...
							<div class="row g-2 mt-2">
								<div class="col-12 col-md-6">
									...
									<input id="coneScale" class="form-control form-control-sm" type="number" step="0.1" value="0.2">
								</div>
								<div class="col-12 col-md-6">
		...
		<script>
			...
			function render(points, meta){
				...
				const traceBaseline = { type:"scatter3d", mode:"markers", name:"Baseline", x:x0, y:y0, z:z0, marker:{size:4,color:"rgba(15,23,42,.55)"}, text:hover0, hoverinfo:"text" }

				const traces = [traceBaseline]

				if(moved.length > 0){
					const traceLines = { type:"scatter3d", mode:"lines", name:"Displacement", x:lineX, y:lineY, z:lineZ, line:{width:3}, opacity:0.75, hoverinfo:"skip" }
					const traceDeformed = { type:"scatter3d", mode:"markers", name:"Hasil", x:x1, y:y1, z:z1, marker:{size:6,color:lin,colorscale:"Turbo",colorbar:{title:"Linear"}}, text:hover1, hoverinfo:"text" }
					const traceCone = { type:"cone", name:"Vector", x:moved.map(p=>p.e0), y:moved.map(p=>p.n0), z:moved.map(p=>p.z0), u:u, v:v, w:w, anchor:"tail", sizemode:"absolute", sizeref:Math.max(maxLin*coneScale,0.01), showscale:false, opacity:0.85, hoverinfo:"skip" }
					traces.push(traceLines, traceDeformed, traceCone)
				}

				const traceRTS = { type:"scatter3d", mode:"markers+text", name:"RTS", x:[RTS_E], y:[RTS_N], z:[RTS_Z], marker:{size:9,symbol:"diamond",color:"rgba(239,68,68,.95)"}, text:["RTS"], textposition:"top center", hoverinfo:"skip" }
				...
				const dirLineN = { type:"scatter3d", mode:"lines", showlegend:false,
								  x:[cx, cx], y:[cy, cy+L], z:[cz, cz],
								  line:{width:5, color:"#B30000"}, hoverinfo:"skip"
								 }
				const dirLineE = { type:"scatter3d", mode:"lines", showlegend:false,
								  x:[cx, cx+L], y:[cy, cy], z:[cz, cz],
								  line:{width:5, color:"#000000"}, hoverinfo:"skip"
								 }

				const dirLineS = { type:"scatter3d", mode:"lines", showlegend:false,
								  x:[cx, cx], y:[cy, cy-L], z:[cz, cz],
								  line:{width:5, color:"#000000"}, hoverinfo:"skip"
								 }
				const dirLineW = { type:"scatter3d", mode:"lines", showlegend:false,
								  x:[cx, cx-L], y:[cy, cy], z:[cz, cz],
								  line:{width:5, color:"#000000"}, hoverinfo:"skip"
								 }

				const dirText = { type:"scatter3d", mode:"text", showlegend:false,
