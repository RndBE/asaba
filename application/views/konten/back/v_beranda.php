									...
									<div class="card-body">
										<div class="row mb-3 gy-3 justify-content-center">
											...
											<div class="col-md-4 col-lg-3 col-xl-2">
												<div class="card h-100">
													<div class="card-body d-flex flex-column justify-content-center text-center py-3">
														<a href="<?= base_url() ?>adr/set_dashlogger?tabel=adr&id_param=328"><h3 class="fw-normal mb-1">Power RTS</h3></a>
											...
											<div class="col-md-4 col-lg-3 col-xl-2">
												<div class="card h-100">
													<div class="card-body d-flex flex-column justify-content-center text-center py-3">
														<a href="<?= base_url() ?>adr/set_dashlogger?tabel=adr&id_param=152"><h3 class="fw-normal mb-1">Humidity Logger</h3></a>
											...
											<div class="col-md-4 col-lg-3 col-xl-2">
												<div class="card h-100">
													<div class="card-body d-flex flex-column justify-content-center text-center py-3">
														<a href="<?= base_url() ?>adr/set_dashlogger?tabel=adr&id_param=153">	<h3 class="fw-normal mb-1">Battery Logger</h3></a>
											...
											<div class="col-md-4 col-lg-3 col-xl-2">
												<div class="card">
													<div class="card-body d-flex flex-column justify-content-center text-center py-3">
														<a href="<?= base_url() ?>adr/set_dashlogger?tabel=adr&id_param=154"><h3 class="fw-normal mb-1">Temperature Logger</h3></a>
													...
													<div class="card-header py-2 pe-0">
														<div class="row justify-content-between w-100 gy-2">
															...
															<div class="col-lg-auto justify-content-between d-flex">
																...
																<a href="<?= base_url() ?>beranda/view_3d" class="btn btn-outline-primary py-2 me-3"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-rotate-3d"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M12 3a7 7 0 0 1 7 7v4l-3 -3" /><path d="M22 11l-3 3" /><path d="M8 15.5l-5 -3l5 -3l5 3v5.5l-5 3z" /><path d="M3 12.5v5.5l5 3" /><path d="M8 15.545l5 -3.03" /></svg>Lihat 3D</a>
																<form method="post" action="<?= base_url() ?>beranda/export_excel">
																	...
																	<button type="submit" class="btn btn-outline-teal py-2 me-3"><svg  xmlns="http://www.w3.org/2000/svg"  width="24"  height="24"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-file-download"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M14 3v4a1 1 0 0 0 1 1h4" /><path d="M17 21h-10a2 2 0 0 1 -2 -2v-14a2 2 0 0 1 2 -2h7l5 5v11a2 2 0 0 1 -2 2z" /><path d="M12 17v-6" /><path d="M9.5 14.5l2.5 2.5l2.5 -2.5" /></svg>Download Excel</button></form>
																<div class="d-flex rounded border" style="overflow:hidden">
																	<button class="bg-transparent border-0 p-0 m-0" onclick="switch_mode('tabel')"><div id="btn-tabel" class="border-end px-3 py-2 bg-primary fw-bold text-white">Tabel</div></button>
																	<button  class="bg-transparent border-0 p-0 m-0" onclick="switch_mode('peta')"><div id="btn-peta" class="px-3 py-2">Peta</div></button>
																</div>
															</div>
														</div>
													</div>
															...
															<table class="table table-bordered">
																<thead>
																	<tr>
																		...
																		<th class="text-center" colspan="6">Hasil Pengukuran</th>																		 <th class="text-center" colspan="4">Pergeseran</th>
...
console.log('Symbol', L.Symbol);
  ...
  function initLeafletMap() {
    ...
    const map = L.map('map', { zoomControl: true }).setView([parseFloat(lat), parseFloat(lon)], zoom);
    ...
    const baseLayers = { 'Road': road, 'Satellite': satellite };
    ...
    const bindOrUnbindLabel = (m, text) => {
      ...
      if (z >= 14) {
        if (!m.getTooltip()) {
          m.bindTooltip(text, { permanent: true, direction: 'top', className: labelClass(), offset: [0, -6] });
        } else {
    ...
    const updateAllLabels = () => {
      markersWithMeta.forEach(({ marker, label }) => bindOrUnbindLabel(marker, label));
    };
    ...
    location_new.forEach((location) => {
      ...
      markersWithMeta.push({ marker, label: title });
      ...
      if (hasNew) {
        ...
        const mainLine = L.polyline([start, extendedEnd], {
		  ...
		  const line = L.polyline([start, extendedEnd], { color: '#007bff', weight: 1, opacity: 1 }).addTo(map)
		  ...
		  L.polylineDecorator(line, {
			  patterns: [{
				  ...
				  symbol: L.Symbol.arrowHead({
					  pixelSize: 10,
					  polygon: true,
					  pathOptions: { color: '#007bff', weight: 2, fillOpacity: 1 }
				  })
			  }]
		  }).addTo(map)
...
<script>
	...
	$(function () {
		...
		function renderPager() {
			$pager.empty();
			$pager.append($('<li/>',{class:'page-item','data-role':'prev'}).append($('<a/>',{href:'#',class:'page-link',html:svgLeft()})));
			for (let i=1;i<=totalPages;i++){
				$pager.append($('<li/>',{class:'page-item','data-page':i}).append($('<a/>',{href:'#',class:'page-link',text:i})));
			}
			$pager.append($('<li/>',{class:'page-item','data-role':'next'}).append($('<a/>',{href:'#',class:'page-link',html:svgRight()})));
