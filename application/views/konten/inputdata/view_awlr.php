<?php
$data20 =  $this->db->query('select count(DISTINCT waktu) as waktu from awlr where code_logger="'.$this->session->userdata('log_awlr').'" and waktu >= "'.  $this->session->userdata('tgl_awlr').'  00:00" and  waktu <= "'.  $this->session->userdata('tgl_awlr').'  23:59" ')->row();
$current_time = time();
$current_minute = date('i', $current_time);
$total_minutes = ((int)date('H', $current_time) * 60) + (int)$current_minute;
$data_count = $data20->waktu;
if ($this->session->userdata('tgl_awlr') == date('Y-m-d')) {
	$tgl = date('Y-m-d H:i');
